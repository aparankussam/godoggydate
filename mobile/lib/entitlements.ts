...
// users/{uid}/private/entitlements and are written ONLY by the Stripe webhook
// (firestore.rules blocks client writes). Two grants live here:
...
// The client may READ this doc (its own), never write it — so the app reacts
// instantly to a grant, but the gate itself is always server-authored.
 ...
 * Opened in an external browser (never in-app) — see lib/pro.ts for the same
 * rationale applied to the Pro subscription checkout.
 */
