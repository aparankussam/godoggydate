// mobile/lib/useProEntitlement.ts
// Mirrors web/lib/useProEntitlement.ts. One hook every surface uses to ask "is
// this user Pro right now?" — backed by the realtime entitlements listener, so
// a successful checkout flips the UI the instant the webhook writes the grant
// (no reload). The gate is still enforced server-side in the AI routes; this is
// only for showing/hiding UI.
